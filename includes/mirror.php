...
/**
 * Give search results an image, when the theme asks for a thumbnail.
 *
 * Mirror posts have no attachment; the thumbnail is a URL in the row. Themes
 * that call the_post_thumbnail() get an <img> from here; themes that check
 * has_post_thumbnail() first will simply not show one.
 *
 * @param string $html    Thumbnail HTML.
 * @param int    $post_id Post ID.
 * @return string
 */
function isg_filter_mirror_thumbnail( $html, $post_id ) {
	...
	$src = isg_first( array( $row['content'], $row['thumb'] ) );
	if ( '' === $src ) {
		return $html;
	}
	return sprintf(
		'<img src="%s" alt="%s" loading="lazy" decoding="async" class="isg-search-thumbnail wp-post-image" />',
		esc_url( isg_flickr_sized( $src, 'n' ) ),
		esc_attr( isg_row_alt( $row ) )
	);
}
add_filter( 'post_thumbnail_html', 'isg_filter_mirror_thumbnail', 10, 2 );
